...
import { SignupRequest, SignupResponse, ApiErrorResponse, User, OnboardingStatus } from "../../models/user";
...
export class AuthService {
    ...
    static async signup(userData: SignupRequest): Promise<SignupResponse> {
        try {
            ...
            let errorMessage = errorResponse?.message || 'Registration failed. Please try again.';
            if (Array.isArray(errorMessage)) {
                errorMessage = errorMessage.join(', ');
            }
            if (statusCode === 409 || (errorMessage.includes('email') && errorMessage.includes('exist'))) {
    ...
    static async updateUserType(userId: string, user: Partial<User>): Promise<User> {
        const response: AxiosResponse<User> = await apiClient.patch(`/user/${userId}`, user);
        return response.data;
    }
    ...
    static async checkOnboardingStatus(): Promise<OnboardingStatus> {
        try {
            ...
            return {
                needsUserType,
                needsChama,
                activeUserType: user.activeUserType ? this.normalizeUserType(user.activeUserType) : null,
