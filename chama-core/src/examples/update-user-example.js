...
function updateUser(userId, updateData, token) {
  return __awaiter(this, void 0, void 0, function* () {
    try {
      ...
      const response = yield axios_1.default.patch(
        `/user/${userId}`,
        updateData,
        {
          headers: {
      ...
      if (axios_1.default.isAxiosError(error)) {
        ...
        if (axiosError.response) {
          ...
          switch (statusCode) {
            case 400:
              throw new Error(
                `Bad request: ${errorData.message || 'Invalid update data'}`,
              );
            case 401:
              throw new Error(
            ...
            case 403:
              throw new Error(
                'Forbidden: You are not authorized to update this user',
              );
            case 404:
              throw new Error(`User with ID ${userId} not found`);
            default:
              throw new Error(
                `Server error: ${errorData.message || 'Failed to update user'}`,
              );
          }
