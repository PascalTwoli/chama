...
let InviteService = (InviteService_1 = class InviteService {
  ...
  createInvite(createInviteDto_1, requestUserId_1) {
    return __awaiter(
      ...
      arguments,
      void 0,
      function* (createInviteDto, requestUserId, sendEmail = false) {
        ...
        try {
          ...
          const isUserAdmin = chama.memberships.some(
            membership => membership.role === client_1.UserRole.CHAIRPERSON,
          );
          this.logger.debug(
          ...
          if (!isUserAdmin) {
            throw new common_1.UnauthorizedException(
              'Only chama admins/chairpersons can send invites',
            );
          }
  ...
  validateAndAcceptInvite(token, userId) {
    return __awaiter(this, void 0, void 0, function* () {
      try {
        ...
        const result = yield this.prisma.$transaction(prisma =>
  ...
  listPendingInvites(chamaId, requestUserId) {
    return __awaiter(this, void 0, void 0, function* () {
      ...
      const isChamaAdmin = chama.memberships.some(
        membership => membership.role === client_1.UserRole.ADMIN,
      );
  ...
  sendInviteEmail(email, chamaName, token, inviterName) {
    return __awaiter(this, void 0, void 0, function* () {
      ...
      try {
        const emailSent = yield this.emailService.sendInviteEmail(
          email,
          chamaName,
          token,
          inviterName,
        ...
        if (emailSent) {
          this.logger.log(
            `Invite email sent to ${email} for chama ${chamaName}`,
          );
          return true;
        } else {
          this.logger.warn(
