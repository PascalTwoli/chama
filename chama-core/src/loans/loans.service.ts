...
import { LoanStatus, transaction_type, transaction_status } from '@prisma/client';
...
export class LoansService {
  ...
  async requestLoan(dto: RequestLoanDto, currentUserId: string): Promise<LoanResponseDto> {
    await this.assertMembership(dto.chamaId, currentUserId);

    if (dto.amount <= 0) throw new BadRequestException('Loan amount must be greater than 0');
    if (dto.durationMonths < 1 || dto.durationMonths > 60) {
      throw new BadRequestException('Duration must be between 1 and 60 months');
    }

    const defaulted = await this.repository.findDefaultedLoan(dto.chamaId, currentUserId);
    if (defaulted) throw new BadRequestException('You have a defaulted loan. New requests are not allowed.');

    const active = await this.repository.findActiveLoan(dto.chamaId, currentUserId);
    if (active) throw new BadRequestException('You already have an active or pending loan in this chama.');

    const referenceCode = await this.repository.generateReferenceCode(dto.chamaId);
    ...
    await this.sendNotification('loan.request', {
      chamaId: dto.chamaId,
  ...
  async createLoan(dto: CreateLoanDto, currentUserId: string): Promise<LoanResponseDto> {
    ...
    const allowedStatuses: LoanStatus[] = [LoanStatus.APPROVED, LoanStatus.DISBURSED, LoanStatus.REQUESTED];
    if (!allowedStatuses.includes(targetStatus)) {
      throw new BadRequestException('Admin creation only supports REQUESTED, APPROVED, or DISBURSED status');
    }
    ...
    if (targetStatus === LoanStatus.DISBURSED || targetStatus === LoanStatus.APPROVED) {
      if (dto.startDate) {
    ...
    const loan = await this.prisma.$transaction(async tx => {
      const created = await tx.loan.create({
        data: {
          ...
          approvedBy: targetStatus !== LoanStatus.REQUESTED ? currentUserId : undefined,
      ...
      if (targetStatus === LoanStatus.DISBURSED) {
        await this.createTreasuryTransaction(
          tx, dto.chamaId, currentUserId, dto.amount,
          transaction_type.LOAN, `Loan disbursed: ${referenceCode}`, referenceCode,
        );
      }
  ...
  async findMany(query: LoanQueryDto, currentUserId: string): Promise<PaginatedLoansDto> {
    if (query.chamaId) {
  ...
  async reviewLoan(id: string, currentUserId: string): Promise<LoanResponseDto> {
    const loan = await this.getLoanOrThrow(id);
    await this.assertPermission(loan.chamaId, currentUserId, 'loan.review');

    if (loan.status !== LoanStatus.REQUESTED) {
      throw new BadRequestException(`Loan must be in REQUESTED status to review. Current: ${loan.status}`);
    }
  ...
  async approveLoan(id: string, dto: ApproveLoanDto, currentUserId: string): Promise<LoanResponseDto> {
    const loan = await this.getLoanOrThrow(id);
    await this.assertPermission(loan.chamaId, currentUserId, 'loan.approve');

    const approvableStatuses: LoanStatus[] = [LoanStatus.REQUESTED, LoanStatus.UNDER_REVIEW];
    if (!approvableStatuses.includes(loan.status)) {
      throw new BadRequestException(`Loan cannot be approved from status: ${loan.status}`);
    }
    if (dto.approvedAmount <= 0) throw new BadRequestException('Approved amount must be greater than 0');
    ...
    await this.sendNotification('loan.approved', {
  ...
  async rejectLoan(id: string, dto: RejectLoanDto, currentUserId: string): Promise<LoanResponseDto> {
    const loan = await this.getLoanOrThrow(id);
    await this.assertPermission(loan.chamaId, currentUserId, 'loan.reject');

    const rejectableStatuses: LoanStatus[] = [LoanStatus.REQUESTED, LoanStatus.UNDER_REVIEW, LoanStatus.APPROVED];
    if (!rejectableStatuses.includes(loan.status)) {
      throw new BadRequestException(`Loan cannot be rejected from status: ${loan.status}`);
    }
    ...
    await this.sendNotification('loan.rejected', {
      chamaId: loan.chamaId,
      title: 'Loan Rejected',
  ...
  async disburseLoan(id: string, dto: DisburseLoanDto, currentUserId: string): Promise<LoanResponseDto> {
    const loan = await this.getLoanOrThrow(id);
    ...
    if (loan.disbursedAt) throw new BadRequestException('Loan has already been disbursed');
    ...
    const updated = await this.prisma.$transaction(async tx => {
      const disbursed = await this.repository.updateStatus(id, {
      ...
      await this.createTreasuryTransaction(
        tx, loan.chamaId, currentUserId, approvedAmount,
        transaction_type.LOAN, `Loan disbursed: ${loan.referenceCode}`, loan.referenceCode,
      );

      return disbursed;
    });

    await this.sendNotification('loan.disbursed', {
      chamaId: loan.chamaId,
  ...
  async recordRepayment(
    ...
    const loan = await this.getLoanOrThrow(id);
    await this.assertPermission(loan.chamaId, currentUserId, 'loan.repayment.record');

    const repayableStatuses: LoanStatus[] = [LoanStatus.ACTIVE, LoanStatus.OVERDUE, LoanStatus.DISBURSED];
    ...
    if (currentBalance <= 0) throw new BadRequestException('Loan has no outstanding balance');
    if (dto.amount <= 0) throw new BadRequestException('Repayment amount must be greater than 0');
    ...
    const updated = await this.prisma.$transaction(async tx => {
      await this.repository.createRepayment({
        loanId: id,
      ...
      const loanUpdate = await this.repository.updateStatus(id, {
      ...
      await this.createTreasuryTransaction(
        tx, loan.chamaId, currentUserId, effectiveAmount,
        transaction_type.LOAN_REPAYMENT,
        `Loan repayment: ${loan.referenceCode}`, loan.referenceCode,
      );

      return loanUpdate;
    });

    await this.sendNotification('loan.repayment.recorded', {
      chamaId: loan.chamaId,
    ...
    return this.mapToDto(await this.repository.findById(id) as LoanWithRelations);
  }
  ...
  async markDefaulted(id: string, currentUserId: string): Promise<LoanResponseDto> {
    const loan = await this.getLoanOrThrow(id);
    await this.assertPermission(loan.chamaId, currentUserId, 'loan.mark.defaulted');

    const defaultableStatuses: LoanStatus[] = [LoanStatus.ACTIVE, LoanStatus.OVERDUE];
    if (!defaultableStatuses.includes(loan.status)) {
      throw new BadRequestException(`Loan must be ACTIVE or OVERDUE to be marked as defaulted. Current: ${loan.status}`);
    }
    ...
    await this.sendNotification('loan.defaulted', {
      chamaId: loan.chamaId,
  ...
  async getStats(chamaId: string, currentUserId: string): Promise<LoanStatsDto> {
    await this.assertMembership(chamaId, currentUserId);
    await this.repository.markOverdueLoans(chamaId);
    return this.repository.getStats(chamaId);
  }
  ...
  private async assertMembership(chamaId: string, userId: string): Promise<void> {
    ...
    if (!membership) throw new ForbiddenException('You are not a member of this chama');
  }

  private async assertPermission(chamaId: string, userId: string, permissionKey: string): Promise<void> {
    ...
    const hasPermission = memberRole?.role.role_permissions.some(
      rp => rp.permission.key === permissionKey || rp.permission.key === 'issue_loans',
    );
    if (!hasPermission) throw new ForbiddenException(`Permission '${permissionKey}' required`);
  }
  ...
  private async sendNotification(
    ...
    try {
      ...
      this.logger.warn(`Notification '${typeKey}' failed: ${err instanceof Error ? err.message : 'Unknown error'}`);
    }
  }

  private mapToDto(loan: LoanWithRelations): LoanResponseDto {
    const repayments: LoanRepaymentResponseDto[] = (loan.repayments ?? []).map(r => ({
      id: r.id,
      loanId: r.loanId,
    ...
    return {
      ...
      approvedAmount: loan.approvedAmount != null ? Number(loan.approvedAmount) : undefined,
      ...
      interestAmount: loan.interestAmount != null ? Number(loan.interestAmount) : undefined,
      totalPayable: loan.totalPayable != null ? Number(loan.totalPayable) : undefined,
