...
let TransactionController = class TransactionController {
    ...
    async createTransaction(createTransactionDto, currentUser) {
        try {
    ...
    async getTransactionsByChama(chamaId, type, startDate, endDate, currentUser) {
        try {
            if (!currentUser) {
                throw new Error('User not authenticated');
            }
            return await this.transactionService.getTransactionsByChama(chamaId, currentUser.id, type, startDate, endDate);
        }
        catch (error) {
            if (error instanceof common_1.NotFoundException) {
                throw error;
    ...
    async getUserTransactionSummary(currentUser) {
        try {
